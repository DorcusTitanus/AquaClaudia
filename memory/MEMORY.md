...
- [project_niche_app_strategy.md](project_niche_app_strategy.md) — Tactical playbook for a portfolio of cheap niche iOS utilities (¡Poof! is the template): iOS-only native SwiftUI, free core + $4.99 honor unlock, audience-led discovery, 4-point idea filter. Derived from reference_app_economics_2026.
- [project_poof_matting.md](project_poof_matting.md) — How ¡Poof! got a background remover that beats Apple's: BiRefNet (MIT) → Core ML on-device. The MIT-license choice, the two conversion blockers (torch.export + grid_sample deform-conv), and the one-line CPU-only OOM fix (1.3GB→181MB). Private repo DorcusTitanus/poof-ios.
- [reference_app_economics_2026.md](reference_app_economics_2026.md) — 2026 market research (fact-checked, 23 sources) on cheap niche app economics: upfront-paid is low-ceiling, iOS ~2.4x Android revenue, discovery (not building) is the wall, top 1% take 92% of IAP. Backs the strategy playbook.
...
- [project_apc_mini_busking_layout.md](project_apc_mini_busking_layout.md) — APC mini mk2 → QLC+ busking layout. Mood-based looks per genre/event page, momentary +15% intensity bumps as strobes, audio-triggered tempo planned. Design intent; reconcile with shai-hulud's actual built .qxw.
- [reference_fhc_stage_geometry.md](reference_fhc_stage_geometry.md) — Farmhouse Collective stage survey (laser-measured June 2026): stage, trusses, LED, barn doors, amp closet, sub boxes. For Blender model + AI controller rig geometry + livestream blocking. Flagged sanity-checks + still-needed list inside.
- [project_fhc_previz.md](project_fhc_previz.md) — **Home for FHC previsualization** — Blender build (FHC_Stage.blend) + QLC+→Art-Net→BlenderDMX link, PROVEN end-to-end 2026-06-09. CALIBRATED against the real rig 2026-06-12 (trusses re-heighted, rear X anchored, all 3 aims converge). Full debugging + calibration log inside.
- [project_video_dj_lightshow.md](project_video_dj_lightshow.md) — Vision: "video DJ" performance role — pixel-mapped music video → wash colors + tempo flashes, on the FHC previz + QLC+ XML agent-loop pipeline.
- [feedback_respect_physical_constraints.md](feedback_respect_physical_constraints.md) — Physical constraints override solver math: truss-hung fixtures share Y/Z, only X varies — don't move them per-fixture to chase noisy triangulation.
...
- [project_bekins_room_map.md](project_bekins_room_map.md) — Maps all 196 Insta360 X4 clips (2026-05-18 capture) to rooms; clip# → `_00_NNN` file. Priority focus: Jimmy Room + foyer + common room + central hallway
- [feedback_verify_compute_target_first.md](feedback_verify_compute_target_first.md) — GPU/accelerator-bound jobs: confirm the target machine has CUDA vs Metal BEFORE picking tools or moving data; re-read project memory for already-chosen tools first
- [feedback_diagnose_dont_guess.md](feedback_diagnose_dont_guess.md) — Troubleshooting: find the REAL cause (re-check my own prior flags first, verify before advising) — don't spray guesses; it wasted ~2hr of Jeff's time on a TestFlight issue I'd already flagged. One cause + how to confirm; stop guessing by the 2nd hypothesis.
- [reference_splat_quality_2026.md](reference_splat_quality_2026.md) — What makes a great Gaussian splat: failure modes (floaters/aliasing/geometry/specular/bloat) + SOTA fixes, fidelity metrics, and the Apple-Silicon reality (SOTA is mostly CUDA-locked; capture+poses matter most on M5/Brush). For Bekins + Splat Haus
...
- [project_chronical.md](project_chronical.md) — ChroniCal: macOS SwiftUI app wrapping Python calendar pipeline for Chronicle Books. Private repo at `github.com/DorcusTitanus/happenstance`. **v1.1 shipped 2026-05-16** (three pushes same day from v0.5 baseline): lint review pane, comprehensive pre-checks, in-app Help/FAQ window (⌘?), Save/Save As, ⌘+/− text zoom, docx-derived output naming, Holiday-mismatch reclassification (advisories vs HolidaySpecial-suggestion vs hard-error), Daily WCS-style options, lenient DateSlug parsing. Verified structurally identical to production WCS 2027 output pre-hand-curation. Jefe quote: "FUCKING AMAZING. 20-year bluesky dream realized."
...
- [reference_chronicle_frontmatter.md](reference_chronicle_frontmatter.md) — Per-product front-matter structure from 7 years of PDFs: branded cover is always a separate CVR; **Wall interiors open with a page-1 art image BEFORE the previous-year block** (standard, not an editorial mistake). Daily/Planner/Engagement/Family Wall each differ.
- [feedback_content_is_king.md](feedback_content_is_king.md) — The docx→XML roundtrip exists to STOP content loss (old regex factories dropped content via greedy matches). Every content-bearing docx paragraph must reach the input XML — misplaced/flagged OK, dropped is not. Indicts `DocxBodyBuilder.preambleStart()`'s silent discard.
- [project_happenstance_typesetting_pipeline.md](project_happenstance_typesetting_pipeline.md) — Current 4-stage Happenstance pipeline: Word→XML perl pre, manual InDesign layout, JSX export with returnfinder XSL (strips U+2028/9/8233 at source), 166KB perl post-process → epub/Word. Swift rewrite replaces stages 1+4; InDesign stays. Companion to [[chronical-paradigms]].
- [project_happenstance_post_process_findings.md](project_happenstance_post_process_findings.md) — Top-line findings from agent-mapped post_process_NABv05.pl: only epub+MSWord paths actually reachable; WileyML/ALL dead; 3 subs (postEPUB, postWileyML, build_the_epub) = 25% of port effort; sentinel-placeholder + stateful-global antipatterns to drop in Swift. Detail catalog at ~/ClaudeCode/happenstance-typesetting-notes/.
- [project_happenstance_client_dupe_pattern.md](project_happenstance_client_dupe_pattern.md) — Per-client converters are produced today by forking+editing the previous client's dir. No real abstraction. Swift rewrite must replace with first-class client-config (NAB, CCC, long-tail). PRE cares about trim size, POST doesn't.
- [project_happenstance_app_architecture.md](project_happenstance_app_architecture.md) — **The Swift "Happenstance App" architecture anchor.** One app, all clients, ChronCal-shaped GUI. 5 decisions locked 2026-05-27: configs on Sync.com; client = top-level toggle; PRE+POST unified; Swift orchestrates InDesign via AppleScript→JSX (no porting); comp-initiated "start post-process" button. Jobs are long-lived (days to month+).
- [project_publishing_decline_timeline.md](project_publishing_decline_timeline.md) — Running "death of the written word" thread Jefe wants built incrementally. Entries: 2013 Wiley offshoring; ~2022–2023 No Starch (Bill Pollock) → Overleaf; Spring 2023 Amazon stops warehousing US books in EU. Pattern: big-player cost cuts (Wiley/Amazon) or tooling consolidation (No Starch) whose externalities land on small operators.
- [user_wiley_history.md](user_wiley_history.md) — Wiley was Happenstance's biggest client for ~13 years, lost in 2013 offshoring to India. "Fucking evil"; OER FTW. **Do NOT preserve WileyML in Swift rewrite; do NOT suggest big-publisher pitches.**
- [reference_epubcheck_workflow.md](reference_epubcheck_workflow.md) — How to run epubcheck 4.2.1 (jar at NorthAtlanticConverter/.../xsl/). `--mode exp` for unzipped trees. Repack with `zip -X0` mimetype first, stored uncompressed.
- [reference_epub_failure_modes_nab.md](reference_epub_failure_modes_nab.md) — Seed-case catalog for the future Happenstance App epub validator: U+2028 in xhtml, mis-nested `<li>` at part boundaries cascading downstream errors, manifest items missing from spine, endnote asides emitted without `id` attrs. Real cases observed 2026-05-27.
...
- [reference_indesign_scripts.md](reference_indesign_scripts.md) — Running collection of ExtendScript (.jsx) tools installed in InDesign's Scripts Panel for comp-side workflow shortcuts. Started 2026-05-22 with BridgeToNextPage.jsx. Conventions, known issues, "how to add a new one" notes. Expect this list to grow.
